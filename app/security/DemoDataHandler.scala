...
class DemoDataHandler extends DataHandler[User] {
  ...
  def findClientUser(clientCredential: ClientCredential, scope: Option[String]): Future[Option[User]] = {
    Future(Some(User(1, "demoUser", "fakePassword")))
  }
  ...
  def findAuthInfoByAccessToken(accessToken: AccessToken): Future[Option[AuthInfo[User]]] = Future(Option(AuthInfo(User(1, "demoUser", "fakePassword"), None, None, None)))
